...
namespace MoeLoaderP.Core.Sites
{
    ...
    public class PixivSite : MoeSite
    {
        ...
        private async Task LoginAsync()
        {
            ...
            {
                Extend.Log(e);
                Extend.ShowMessage("无法连接至https://www.pixiv.net，请检查代理设置");
                return;
            }

            // 步骤1 GET 获取 post_key 参数
            Extend.ShowMessage("pixiv.net自动登录中",1);
            ...
            {
                Extend.Log(ShortName, "自动登录失败(获取post_key失败）");
                Extend.ShowMessage("pixiv.net自动登录失败");
                return;
            }
            ...
            try
            {
                ...
                {
                    IsLogin = true;
                    Extend.ShowMessage("pixiv.net登录成功，获取页面中", 1);
                }
            ...
            else
            {
                ...
                switch (SearchType)
                {
                    case SearchTypeEnum.Author:// 作者
                        ...
                        {
                            Extend.ShowMessage("参数错误，必须在关键词中指定画师 id（纯数字）");
